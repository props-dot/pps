...
if st.button("Run custom backtest", type="primary", key="run_custom_backtest"):
    if not ALPACA_KEY or not ALPACA_SECRET:
        ...
        load_status = st.status("Loading historical Alpaca data...", expanded=True)

        try:
            all_bt_df = fetch_backtest_range(
                tuple(symbols),
                calendar_start.date(),
                calendar_end.date(),
                feed,
            ...
            load_status.update(label="Market-data load failed", state="error")
            st.error(str(e))
            st.stop()

        if all_bt_df.empty:
            load_status.update(label="No market data returned", state="error")
            st.error("No Alpaca market data was returned for the requested period.")
            st.stop()

        load_status.write(
            f"Loaded {len(all_bt_df):,} one-minute bars across "
            f"{all_bt_df['symbol'].nunique()} symbols."
        )
        load_status.write("Indexing stock/day sessions...")
        session_map = build_session_map(all_bt_df)
        load_status.update(label="Historical data loaded", state="complete", expanded=False)

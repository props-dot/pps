...
st.title("One-Hour Stock Strategy — V7")
...
def load_history_from_github():
    """
    Returns (history_df, sha). A 404 means the source file does not exist yet.
    """
    if not GITHUB_TOKEN:
    ...
    return _normalize_history(df), payload.get("sha")
...
def ensure_persistent_history(
    ...
    if requested_existing.empty:
        if status_box:
            status_box.write("No saved market history found. Creating initial source data...")
        updates.append(

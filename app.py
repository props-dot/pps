...
st.title("One-Hour Stock Strategy — V8")
st.caption("Persistent-data edition with clearer rankings, compounding portfolio backtests, and on-demand optimizers.")
...
def optimize_triggers_for_subjects(session_map, subject_days, top10_by_day,
    ...
    """
    Hindsight optimizer over a supplied set of subject days and precomputed Top-10 selections.
    The Top-10 selection itself is never changed by the trigger grid.
    """
...
with st.expander("30-session Historical Top-10 optimizer", expanded=False):
    ...
    if st.button("Run 30-session historical optimizer", key="run_hist_optimizer"):
        ...
        with st.spinner("Optimizing S1 and S2 across the historical 30-session window..."):
...
with st.expander(f"Daily optimizer — {latest_date}", expanded=False):
    ...
    if st.button("Run daily 1 / 5 / 15 / 30-day optimizer", key="run_daily_optimizer"):
        with st.spinner("Testing daily lookbacks and 25% trigger increments..."):
...
with p3:
    starting_capital = st.number_input(
        ...
        step=100.0,
        format="%.2f",
        help="S1 splits this equally across the 10 selected stocks. S2 uses the full current balance sequentially."
    )
...
st.info(
    "Each subject day is independently ranked from that subject day's own prior lookback sessions. "
    "For example, lookback=1 means every subject day uses the immediately preceding complete session's Top 10."
)
...
if st.button("Run custom backtest", type="primary", key="run_custom_backtest"):
    if not ALPACA_KEY or not ALPACA_SECRET:
        ...
        for subject_index, subject_day in enumerate(subject_dates):
            ...
            s1_start_balance = s1_balance
            s2_start_balance = s2_balance
            s1_gain_loss = s1_start_balance * (float(s1_ret) / 100.0)
            s2_gain_loss = s2_start_balance * (float(s2_ret) / 100.0)
            s1_balance = s1_start_balance + s1_gain_loss
            s2_balance = s2_start_balance + s2_gain_loss

            if not s1_trades.empty:
                s1_trades["subject_date"] = subject_day
                s1_trades["subject_starting_capital"] = s1_start_balance
                s1_trades["allocated_dollars"] = s1_start_balance / max(1, len(top10))
                s1_trades["trade_dollar_gain_loss"] = (
                    s1_trades["allocated_dollars"] * s1_trades["return_pct"] / 100.0
                )
                all_s1_trades.append(s1_trades)

            if not s2_trades.empty:
                s2_trades["subject_date"] = subject_day
                s2_trades["subject_starting_capital"] = s2_start_balance
        ...
        if results.empty:
            ...
            mc1.metric("S1 ending value", f"${s1_stats['ending_value']:,.2f}", f"{s1_stats['total_return_pct']:.2f}%")
            mc2.metric("S2 ending value", f"${s2_stats['ending_value']:,.2f}", f"{s2_stats['total_return_pct']:.2f}%")
            mc3.metric("S1 avg/day", f"{s1_stats['avg_daily_return_pct']:.3f}%")
            mc4.metric("S2 avg/day", f"{s2_stats['avg_daily_return_pct']:.3f}%")
            mc5.metric("S1 max drawdown", f"{s1_stats['max_drawdown_pct']:.2f}%")
            mc6.metric("S2 max drawdown", f"{s2_stats['max_drawdown_pct']:.2f}%")

            summary_rows = pd.DataFrame([
                {
                    "Scenario": "S1",
                    "Starting Capital": starting_capital,
                    "Ending Capital": s1_stats["ending_value"],
                    "Dollar Gain/Loss": s1_stats["dollar_gain_loss"],
                    "Total Return %": s1_stats["total_return_pct"],
                    "Average Daily Return %": s1_stats["avg_daily_return_pct"],
                    "Winning Days %": s1_stats["win_rate_pct"],
                    "Max Drawdown %": s1_stats["max_drawdown_pct"],
                    "Best Day %": s1_stats["best_day_pct"],
                    "Worst Day %": s1_stats["worst_day_pct"],
                },
                {
                    "Scenario": "S2",
                    "Starting Capital": starting_capital,
                    "Ending Capital": s2_stats["ending_value"],
                    "Dollar Gain/Loss": s2_stats["dollar_gain_loss"],
                    "Total Return %": s2_stats["total_return_pct"],
                    "Average Daily Return %": s2_stats["avg_daily_return_pct"],
                    "Winning Days %": s2_stats["win_rate_pct"],
                    "Max Drawdown %": s2_stats["max_drawdown_pct"],
                    "Best Day %": s2_stats["best_day_pct"],
                    "Worst Day %": s2_stats["worst_day_pct"],
                },
            ])
            ...
            metadata = {
                "Historical lookback sessions": lookback_sessions,
                "Best-window length minutes": best_window_minutes,
                ...
                "Calculated subject dates": len(results),
                "Selection rule": "Each subject day independently uses only its own prior lookback sessions",
            }
            ...
            if rankings:
                ...
                with st.expander("Top 10 selected for each subject day"):
                    ...
                    st.download_button(
                        ...
                        csv_with_metadata(rank_df, metadata),
            ...
            if all_s1_trades:
                ...
                with st.expander("S1 trade log"):
                    ...
                    st.download_button(
                        ...
                        csv_with_metadata(s1df, metadata),
            ...
            if all_s2_trades:
                ...
                with st.expander("S2 trade log"):
                    ...
                    st.download_button(
                        ...
                        csv_with_metadata(s2df, metadata),
...
st.caption(
    ...
    "Lookback=0 contains look-ahead bias and is diagnostic only. Optimizer summaries are retrospective/hindsight benchmarks."
)
